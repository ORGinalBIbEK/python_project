import tkinter
import math

button_value = [
    ["AC", "DEL", "%", "π"],
    ["7",  "8",   "9", "X"],
    ["4",  "5",   "6", "-"],
    ["1",  "2",   "3", "+"],
    ["0",  ".",  "+/-","="]
]

right_symbol = ["X", "-", "+", "="]
top_symbol   = ["AC", "DEL", "%", "π"]
operators    = ["X", "-", "+"]

row_count    = len(button_value)
column_count = len(button_value[0])

# Colors
color_dark_charcoal = "#1C1C1E"
color_light_gray    = "#3A3A3C"
color_bright_color  = "#FF9F0A"
color_slate_gray    = "#A5A5A5"

# Window setup
window = tkinter.Tk()
window.title("Calculator")
window.resizable(False, False)

frame = tkinter.Frame(window, bg=color_dark_charcoal)

# Row 0: expression history (small, gray)
expr_label = tkinter.Label(frame, text="", bg=color_dark_charcoal, fg="gray",
                           font=("Arial", 15), anchor="e", width=column_count)
expr_label.grid(row=0, column=0, columnspan=column_count, sticky="we")

# Row 1: current number (big, white)
label = tkinter.Label(frame, text="0", bg=color_dark_charcoal, fg="white",
                      font=("Arial", 30), anchor="e", width=column_count)
label.grid(row=1, column=0, columnspan=column_count, sticky="we")

# Buttons start at row 2 (not row 1!)
for row in range(row_count):
    for column in range(column_count):
        value  = button_value[row][column]
        button = tkinter.Button(frame, text=value, bg=color_dark_charcoal,
                                fg="white", font=("Arial", 30),
                                width=column_count - 1, height=1,
                                command=lambda value=value: button_clicked(value))
        if value in top_symbol:
            button.config(bg=color_slate_gray)
        elif value in right_symbol:
            button.config(bg=color_bright_color)
        else:
            button.config(bg=color_light_gray)

        button.grid(row=row + 2, column=column, sticky="nsew", padx=1, pady=1)  # row+2 !

frame.pack()

# State
A        = "0"
operator = None

def clear_all():
    global A, operator
    A        = "0"
    operator = None
    expr_label["text"] = ""

def remove_zero_decimal(result):
    if result % 1 == 0:
        return int(result)
    else:
        return result

def parse_display(text):
    return float(text.replace("π", str(math.pi)))

def button_clicked(value):
    global A, operator

    if value in operators:
        A             = label["text"]
        operator      = value
        expr_label["text"] = A + " " + value
        label["text"] = "0"

    elif value == "=":
        if operator is not None:
            try:
                numA = parse_display(A)
                numB = parse_display(label["text"])

                if operator == "X":
                    result = numA * numB
                elif operator == "-":
                    result = numA - numB
                elif operator == "+":
                    result = numA + numB

                expr_label["text"] = A + " " + operator + " " + label["text"] + " ="
                label["text"]      = str(remove_zero_decimal(result))
                A = label["text"]
            except:
                label["text"] = "Error"
            operator = None

    elif value in top_symbol:
        if value == "AC":
            clear_all()
            label["text"] = "0"

        elif value == "DEL":
            text = label["text"]
            if len(text) > 1:
                text = text[:-1]
                if text == "-" or text == "":
                    text = "0"
            else:
                text = "0"
            label["text"] = text

        elif value == "%":
            try:
                result = parse_display(label["text"]) / 100
                label["text"] = str(remove_zero_decimal(result))
            except:
                label["text"] = "Error"

        elif value == "π":
            if label["text"] == "0":
                label["text"] = "π"         # show symbol only
            else:
                label["text"] += "π"

    else:  # digits, dot, +/-
        if value == ".":
            if "." not in label["text"]:
                label["text"] += value

        elif value == "+/-":
            try:
                result = parse_display(label["text"]) * -1
                label["text"] = str(remove_zero_decimal(result))
            except:
                label["text"] = "Error"

        elif value in "0123456789":
            if label["text"] == "0":
                label["text"] = value
            else:
                label["text"] += value

# Center the window
window.update()
window_width  = window.winfo_width()
window_height = window.winfo_height()
screen_width  = window.winfo_screenwidth()
screen_height = window.winfo_screenheight()

window_x = int((screen_width  / 2) - (window_width  / 2))
window_y = int((screen_height / 2) - (window_height / 2))

window.geometry(f"{window_width}x{window_height}+{window_x}+{window_y}")
window.mainloop()